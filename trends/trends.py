...
def tweet_string(tweet):
    ...
    if callable(tweet):
        location = tweet_location_fn(tweet)
        point = (latitude(location), longitude(location))
        return '"{0}" @ {1}'.format(tweet_text_fn(tweet), point)
    else:
...
def extract_words(text):
    ...
    return cleaned_str.split()  # Replace this line
...
def analyze_tweet_sentiment(tweet):
    ...
    for i in words:
        ...
        s = s() if callable(s) else s["value"]
        if s != None:
            score += s
            count += 1
...
def find_centroid(polygon):
    ...
    if len(polygon) >= 2:
        for i in range(len(polygon)-1):
            if callable(polygon[i]):
                x1 = polygon[i](1)
                y1 = polygon[i](0)
            else:
                x1, y1 = polygon[i]

            if callable(polygon[i+1]):
                x2 = polygon[i+1](1)
                y2 = polygon[i+1](0)
            else:
                x2, y2 = polygon[i+1]
...
def find_state_center(polygons):
    ...
    average_cy = 0.0
    for i in polygons:
        cx, cy, area = find_centroid(i)
        all_area += area
        average_cx += area*cx
        average_cy += area*cy
    average_cx /= all_area
    average_cy /= all_area
    return make_position(average_cx, average_cy)
###################################
...
def group_tweets_by_state(tweets):
    ...
    """
    tweets_by_state = {}
    state_center = { i:find_state_center(us_states[i]) for i in us_states.keys()}

    for i in tweets:
        min_distance = None
        closest_state = None
        for s in state_center.keys():
            d = geo_distance(tweet_location(i), state_center[s])
...
def average_sentiments(tweets_by_state):
    ...
    def for_sentiments(x):
        score = 0.0
        num_sentiments = 0
        for i in x:
            sentiments = analyze_tweet_sentiment(i)
            if has_sentiment(sentiments): 
                score += sentiment_value(sentiments)
                num_sentiments += 1

        if num_sentiments != 0:
            return score/num_sentiments
        else:
            return 0.0

    averaged_state_sentiments = {i:for_sentiments(tweets_by_state[i]) for i in tweets_by_state.keys() if len(tweets_by_state[i])>0}
    averaged_state_sentiments = {i: averaged_state_sentiments[i] for i in averaged_state_sentiments.keys() if averaged_state_sentiments[i] != 0.0}
