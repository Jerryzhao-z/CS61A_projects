...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        # swine swap
        if opponent_score != 0 and score != 0 and (score/opponent_score == 2.0 or opponent_score/score == 2.0):
            s = score
        ...
        if who == 0:
            ...
            opponent_score += take_turn(num_rolls, score, dice=dice)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for i in range(10):
        scores[i] = averaged_roll_dice(i+1, dice)
        print (i+1, "dice scores", scores[i], "on average")
        
    num_dice = scores.index(max(scores)) + 1
    return num_dice
...
def swap_strategy(score, opponent_score):
    ...
    if opponent_score != 0 and (score+m+1)/opponent_score == 2.0:
        # harmful
        return BASELINE_NUM_ROLLS
    elif opponent_score/(score+m+1) == 2.0:
        # beneficial
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    "*** YOUR CODE HERE ***"
    s = swap_strategy(score, opponent_score)
    #return s

    if  s != BASELINE_NUM_ROLLS:
        return s
    else:
        if (score+opponent_score+1) % 7 == 0 and opponent_score > score+1:
            return 10
